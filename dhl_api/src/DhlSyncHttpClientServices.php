...
class DhlSyncHttpClientServices
{
   ...
   *   Location as query
   */
    public function getLocationDataByquery(string $query)
    {
        $this->privateTempStore->get('dhl_api') ? $this->privateTempStore->get('dhl_api')->set("yml_render", []) : null;
        try {
            $request_options = [
            'headers' => [
            ...
            $request = $this->httpClient->get(
                "https://api.dhl.com/location-finder/v1/find-by-address?{$query}",
                $request_options
            );

            if ($data = Json::decode($request->getBody())) {
                $locations_filtered = [];
                return !empty($data['locations']) ? $data['locations'] : $data;
            }
   ...
   */
    public function countryLookupBycode(string $query)
    {
        ...
        $data = $this->getLocationDataByquery($query);
        if (!empty($data)) {
            $locations_filtered = [];
            foreach ($data as $eo_key => $eo_value) {
